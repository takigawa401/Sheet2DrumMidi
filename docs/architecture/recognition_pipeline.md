...
flowchart TD
    ...
    RecognitionResult --> ScoreBuilder[ScoreBuilder<br/>未実装]
...
- **出力:** 将来の`ScoreBuilder`が解釈できるProvider非依存の認識結果
...
- **入力:** 1ページ以上の認識結果。複数ページの集約方法を含む具体的なAPIは未定義
- **出力:** 検証済みの`Score`、または後続検証で扱えるエラー
- **責務:** 認識結果を音楽ドメインとして解釈し、楽器ラベルの`DrumInstrument`へのマッピング、欠落情報の扱い、イベント順序の正規化、小節容量や拍子の検証を行う。
- **責務外:** Provider API通信、Prompt生成、画像レンダリング、Provider固有レスポンスの解析。

ScoreBuilderは未実装であるため、本書ではIssue #15・#16が定める責務境界のみを記載する。具体的な補完方針やエラー表現は将来の実装時に決定する。
...
- 欠落したパート名、小節番号、拍子などを補完するか、変換エラーとして扱う。
- イベント順序を必要に応じて正規化する。
- 各イベントが小節容量内に収まることを確認する。
- 拍子分母など、`Score`が要求する音楽ドメイン上の制約を確認する。
- 複数ページの認識結果を楽譜のパート・小節へ統合する。
...
新しい実装が次の契約を守れば、PageRenderer、RenderedPage、RecognitionResult、および将来のScoreBuilderを変更する必要はない。
